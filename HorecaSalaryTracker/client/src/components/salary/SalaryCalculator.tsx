...
export function SalaryCalculator() {
      ...
      tables: [
        ...
        },
        {
          ...
          body: [
            ...
            ['Contract Percentage', `KPI Contract Threshold (${contractPercentage >= kpiSettings.contractThreshold ? contractPercentage : 0}% of ${kpiSettings.contractThreshold}% required)`, `${contractPercentage >= kpiSettings.contractThreshold ? 'Applied' : 'Not Applied'}`],
  ...
  const handleExportToExcel = () => {
    ...
    const kpiSettingsData = [
      ['KPI Settings', '', '', ''],
      ['Setting', 'Value', '', ''],
      ['Representative Name', kpiSettings.representativeName || 'Sales Representative', '', ''],
      ['Target Total Salary', formatCurrency(kpiSettings.totalTargetSalary), '', ''],
      ['Base Salary Percentage', `${kpiSettings.baseSalaryPercentage}%`, '', ''],
      ['', '', '', ''],
      ['Visit Targets', '', '', ''],
      ['Large Cafe Visit Target', kpiSettings.targetLargeVisit, '', ''],
      ['Medium Cafe Visit Target', kpiSettings.targetMediumVisit, '', ''],
      ['Small Cafe Visit Target', kpiSettings.targetSmallVisit, '', ''],
      ['Total Visit Target', visitTargetTotal, '', ''],
      ['Visit Threshold', `${kpiSettings.visitThreshold}%`, '', ''],
      ['', '', '', ''],
      ['Contract Targets', '', '', ''],
      ['Large Cafe Contract Target', kpiSettings.targetLargeContract, '', ''],
      ['Medium Cafe Contract Target', kpiSettings.targetMediumContract, '', ''],
      ['Small Cafe Contract Target', kpiSettings.targetSmallContract, '', ''],
      ['Total Contract Target', contractTargetTotal, '', ''],
      ['Contract Threshold', `${kpiSettings.contractThreshold}%`, '', ''],
      ['', '', '', ''],
      ['Bonuses', '', '', ''],
      ['Large Cafe Bonus', formatCurrency(kpiSettings.largeCafeBonus), 'per contract', ''],
      ['Medium Cafe Bonus', formatCurrency(kpiSettings.mediumCafeBonus), 'per contract', ''],
      ['Small Cafe Bonus', formatCurrency(kpiSettings.smallCafeBonus), 'per contract', ''],
    ];
    ...
    const performanceData = [
      ['Monthly Performance', '', '', ''],
      ['', 'Visits', 'Contracts', ''],
      ['Large Cafes', performance.largeVisits, performance.largeContracts, ''],
      ['Medium Cafes', performance.mediumVisits, performance.mediumContracts, ''],
      ['Small Cafes', performance.smallVisits, performance.smallContracts, ''],
      ['Total Actual', visitActualTotal, contractActualTotal, ''],
      ['Total Target', visitTargetTotal, contractTargetTotal, ''],
      ['Completion Percentage', `${visitPercentage}%`, `${contractPercentage}%`, ''],
      ['Threshold', `${kpiSettings.visitThreshold}%`, `${kpiSettings.contractThreshold}%`, ''],
      ['Threshold Met', visitPercentage >= kpiSettings.visitThreshold ? 'Yes' : 'No', contractPercentage >= kpiSettings.contractThreshold ? 'Yes' : 'No', ''],
    ];
    ...
    const calculationData = [
      ...
      ['Base Salary', `${kpiSettings.totalTargetSalary} × ${kpiSettings.baseSalaryPercentage}%`, formatCurrency(baseSalary), ''],
      ['', '', '', ''],
      ['Contract Bonuses (Before Threshold)', '', '', ''],
      ['Large Cafe Contracts', `${performance.largeContracts} × $${kpiSettings.largeCafeBonus}`, formatCurrency(largeCafeBonus), ''],
      ['Medium Cafe Contracts', `${performance.mediumContracts} × $${kpiSettings.mediumCafeBonus}`, formatCurrency(mediumCafeBonus), ''],
      ['Small Cafe Contracts', `${performance.smallContracts} × $${kpiSettings.smallCafeBonus}`, formatCurrency(smallCafeBonus), ''],
      ['Total Contract Bonuses', 'Sum of all contract bonuses', formatCurrency(totalContractBonus), ''],
      ['', '', '', ''],
      ['Threshold Application', '', '', ''],
      ['Contract Threshold', `${kpiSettings.contractThreshold}%`, '', ''],
      ['Contract Percentage', `${contractPercentage}%`, '', ''],
      ['Contract Threshold Met?', contractPercentage >= kpiSettings.contractThreshold ? 'Yes' : 'No', '', ''],
      ['Applied Contract Bonus', contractPercentage >= kpiSettings.contractThreshold ? formatCurrency(totalContractBonus) : '$0', '', ''],
      ['', '', '', ''],
      ['Visit Threshold', `${kpiSettings.visitThreshold}%`, '', ''],
      ['Visit Percentage', `${visitPercentage}%`, '', ''],
      ['Visit Threshold Met?', visitPercentage >= kpiSettings.visitThreshold ? 'Yes' : 'No', '', ''],
      ['Max KPI Bonus', `${kpiSettings.totalTargetSalary} - ${formatCurrency(baseSalary)} = ${formatCurrency(maxKpiBonus)}`, '', ''],
      ['Remaining Bonus After Contracts', `${formatCurrency(maxKpiBonus)} - ${formatCurrency(totalContractBonus)} = ${formatCurrency(maxKpiBonus - totalContractBonus)}`, '', ''],
      ['Visit Bonus Calculation', `Remaining × (${visitPercentage} ÷ 100)`, '', ''],
      ['Applied Visit Bonus', visitPercentage >= kpiSettings.visitThreshold ? formatCurrency(kpiBonus - totalContractBonus) : '$0', '', ''],
      ['', '', '', ''],
      ['Total KPI Bonus', 'Applied Contract Bonus + Applied Visit Bonus', formatCurrency(kpiBonus), ''],
      ['KPI Bonus as % of Max', `${kpiBonusPercentage}%`, '', ''],
      ['', '', '', ''],
      ['Total Salary', 'Base Salary + KPI Bonus', formatCurrency(totalSalary), ''],
      ['Target Salary', '', formatCurrency(targetSalary), ''],
      ['Achievement', `${totalSalaryPercentage}%`, '', ''],
    ];
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-2 border-t">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className={`h-2.5 rounded-full ${
                          contractPercentage >= (kpiSettings.contractThreshold || 80) 
                            ? 'bg-green-500' 
                            : 'bg-red-500'
                        }`}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    ...
                    <tr>
                      <td className="px-4 py-3 text-sm text-gray-900">Contract Percentage</td>
                      <td className="px-4 py-3 text-sm text-gray-500">
                        Contract Threshold ({contractPercentage}% of {kpiSettings.contractThreshold}% required)
                      </td>
                      <td className="px-4 py-3 text-sm text-right text-gray-900 font-medium">
                        {contractPercentage >= kpiSettings.contractThreshold ? 'Applied' : 'Not Applied'}
                      </td>
                    </tr>
                    <tr>
